/**
 * @styleset/smarthome-glass-theme - Astro Integration
 * Style: custom
 *
 * Usage in astro.config.mjs:
 *   import styleSetTheme from '@styleset/smarthome-glass-theme';
...
export default function styleSetTheme(userOptions = {}) {
  return {
    name: '@styleset/smarthome-glass-theme',
    hooks: {
      'astro:config:setup': ({ injectScript, updateConfig, logger }) => {
        // Inject the theme CSS (custom properties + component styles)
        injectScript('page-ssr', `import '@styleset/smarthome-glass-theme/styles/theme.css';`);

        // Optionally inject reset
        if (userOptions.includeReset !== false) {
          injectScript('page-ssr', `import '@styleset/smarthome-glass-theme/styles/reset.css';`);
        }

/**
 * @styleset/bridgevoice-dark-theme - Astro Integration
 * Style: custom
 *
...
export default function styleSetTheme(userOptions = {}) {
  return {
    name: '@styleset/bridgevoice-dark-theme',
    hooks: {
      'astro:config:setup': ({ injectScript, updateConfig, logger }) => {
        // Inject the theme CSS (custom properties + component styles)
        injectScript('page-ssr', `import '@styleset/bridgevoice-dark-theme/styles/theme.css';`);

        // Optionally inject reset
        if (userOptions.includeReset !== false) {
          injectScript('page-ssr', `import '@styleset/bridgevoice-dark-theme/styles/reset.css';`);
        }

/**
 * @styleset/lumberjack-theme - Astro Integration
 * Style: custom
 *
...
export default function styleSetTheme(userOptions = {}) {
  return {
    name: '@styleset/lumberjack-theme',
    hooks: {
      'astro:config:setup': ({ injectScript, updateConfig, logger }) => {
        // Inject the theme CSS (custom properties + component styles)
        injectScript('page-ssr', `import '@styleset/lumberjack-theme/styles/theme.css';`);

        // Optionally inject reset
        if (userOptions.includeReset !== false) {
          injectScript('page-ssr', `import '@styleset/lumberjack-theme/styles/reset.css';`);
        }

/**
 * @styleset/finance-dashboard-theme - Astro Integration
 * Style: custom
 *
...
export default function styleSetTheme(userOptions = {}) {
  return {
    name: '@styleset/finance-dashboard-theme',
    hooks: {
      'astro:config:setup': ({ injectScript, updateConfig, logger }) => {
        // Inject the theme CSS (custom properties + component styles)
        injectScript('page-ssr', `import '@styleset/finance-dashboard-theme/styles/theme.css';`);

        // Optionally inject reset
        if (userOptions.includeReset !== false) {
          injectScript('page-ssr', `import '@styleset/finance-dashboard-theme/styles/reset.css';`);
        }

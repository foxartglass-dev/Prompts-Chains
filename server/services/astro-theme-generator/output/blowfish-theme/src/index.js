/**
 * @styleset/blowfish-bike-theme - Astro Integration
 * Style: custom
 *
...
export default function styleSetTheme(userOptions = {}) {
  return {
    name: '@styleset/blowfish-bike-theme',
    hooks: {
      'astro:config:setup': ({ injectScript, updateConfig, logger }) => {
        // Inject the theme CSS (custom properties + component styles)
        injectScript('page-ssr', `import '@styleset/blowfish-bike-theme/styles/theme.css';`);

        // Optionally inject reset
        if (userOptions.includeReset !== false) {
          injectScript('page-ssr', `import '@styleset/blowfish-bike-theme/styles/reset.css';`);
        }

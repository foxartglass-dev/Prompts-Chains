...
const App: React.FC = () => {
    ...
    const renderSection = (title: string, id: string, icon: React.ReactNode, children: React.ReactNode, defaultOpen = false) => (
      <div className="bg-gray-800 rounded-lg shadow-lg">
        <h2
          className={`text-xl font-bold flex items-center text-cyan-400 p-6 cursor-pointer select-none hover:bg-cyan-900/20 active:bg-cyan-900/30 transition-colors`}
          onClick={() => toggleCollapsible(id)}
        >
          {/* Left dropdown indicator - clickable square */}
          <div className={`w-7 h-7 mr-3 flex items-center justify-center bg-cyan-500/20 rounded border border-cyan-500/40 transition-transform shrink-0 ${(activeCollapsible === id || (!activeCollapsible && defaultOpen && id === 'setup')) ? 'rotate-180' : ''}`}>

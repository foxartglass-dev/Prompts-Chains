...
const App: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-gray-200 font-sans pt-1 pb-4 px-4 sm:pt-2 sm:pb-6 sm:px-6 lg:pt-2 lg:pb-8 lg:px-8">
             {notification && (
                <div className={`fixed bottom-5 right-20 z-50 px-6 py-3 rounded-xl shadow-card-lg text-white transition-all duration-300 border ${notification.type === 'success' ? 'bg-green-600/90 border-green-500' : notification.type === 'info' ? 'bg-brand-cyan/90 border-brand-cyan-light' : 'bg-red-600/90 border-red-500'}`}>
            ...
            <ArticlesPage
              ...
              defaultWebsiteId={currentWebsiteId}
              workflowId={currentWorkflowId}
            />
            ...
            {/* Link Pool Manager - SEO Link Management */}
            <LinkPoolManager
              isOpen={isLinkPoolOpen}
              onClose={() => setIsLinkPoolOpen(false)}
                        ...
                        <div className="relative flex" data-more-dropdown>
                            ...
                            {isMoreDropdownOpen && (
                                <div className="absolute top-full right-0 mt-2 bg-slate-800 border border-brand-gold rounded-lg shadow-2xl z-50 min-w-[160px]">
                                    ...
                                    <button
                                        onClick={() => { setIsMoreDropdownOpen(false); setIsLinkPoolOpen(true); }}
                                        className="w-full flex items-center gap-2 px-4 py-2.5 text-left text-brand-gold hover:bg-slate-700 transition"
                                    >
                                        <svg className="h-4 w-4 text-brand-cyan" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <main className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col gap-8">
                    ...
                    {/* 11. Component Library - Reusable Page Sections */}
                    {renderSection('11. Component Library', 'componentLibrary',
                        <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">

...
const App: React.FC = () => {
    ...
    const renderSection = (title: React.ReactNode, id: string, icon: React.ReactNode, children: React.ReactNode, defaultOpen = false, rightContent?: React.ReactNode) => (
      <div className="bg-card rounded-xl shadow-glow-cyan card-3d hover:shadow-card-hover border-2 border-brand-cyan relative z-0">
        <h2
          className={`text-xl font-bold flex items-center text-brand-cyan py-3 px-4 cursor-pointer select-none hover:bg-brand-cyan/5 active:bg-brand-cyan/10 transition-colors`}
          onClick={() => toggleCollapsible(id)}
        >
          {/* Left dropdown indicator - clickable square */}
          <div className={`w-7 h-7 mr-3 flex items-center justify-center bg-brand-cyan/20 rounded border border-brand-cyan/40 transition-transform shrink-0 ${openSections.has(id) ? 'rotate-180' : ''}`}>
